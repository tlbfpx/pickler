...
public interface DashboardService {
     ...
     *  数字 KPI 多带 {@code deltaPct}/{@code deltaAbs} 同比/环比）。 */
    ...
    DashboardTrendVO getTrends(String range, String from, String to);

    /** 活动排行。{@code metric} = {@code registrations|revenue|fillRate}。
     *  {@code fillRate} 自动跳过 maxParticipants=0/null；{@code limit} 1..50。 */
    List<TopEventVO> getTopEvents(String metric, String range, String from, String to, int limit);

    /** 出席漏斗：报名/签到/no-show%。registered=0 时 noShowRate=null。 */
    AttendanceFunnelVO getAttendance(String range, String from, String to);

    /** 同比/环比。{@code previous=0,current>0} → deltaPct=null（避免除零与 Infinity）。 */
    CompareResultVO getCompare(String metric, String currentRange, String previousRange);
}
