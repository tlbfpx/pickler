...
// 侧边栏用 el-sub-menu 折叠组：运营管理 / 积分排名 / 内容运营 / 数据中心 / 系统管理
...
test.describe('导航', () => {
  test('侧边栏展示所有菜单项', async ({ adminPage }) => {
    // 运营管理 — 默认随登录展开
    await expect(adminPage.locator('.el-menu-item').filter({ hasText: '运营首页' })).toBeVisible()
    ...
    await expandGroup(adminPage, '积分排名')
    await expect(adminPage.locator('.el-menu-item').filter({ hasText: '积分排名' })).toBeVisible()

    // 内容运营
    await expandGroup(adminPage, '内容运营')
    await expect(adminPage.locator('.el-menu-item').filter({ hasText: '横幅管理' })).toBeVisible()

    // 系统
    await expandGroup(adminPage, '系统管理')
    await expect(adminPage.locator('.el-menu-item').filter({ hasText: '账号管理' })).toBeVisible()
  ...
  test('导航到运营首页', async ({ adminPage }) => {
    await adminPage.locator('.el-menu-item').filter({ hasText: '运营首页' }).click()
