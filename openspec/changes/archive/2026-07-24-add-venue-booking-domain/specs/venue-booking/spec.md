# venue-booking Specification (delta)

## Purpose

Hey Pickler 场馆预约领域契约。覆盖 admin 配置（场馆 / 营业时间 / 联系方式 / 场地 / 定价带）、用户浏览（场地 + 按日格子 + 价格）、预约引擎（下单 / 取消 / 我的）、admin 预约管理（列表 + 完成 / 爽约 / 强制取消），以及并发 CAS 一票否决、BookingSlot 唯一键释放、根因错误鉴别、自动 complete scheduler。
