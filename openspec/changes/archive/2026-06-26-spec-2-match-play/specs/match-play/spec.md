...
The system SHALL provide an endpoint `POST /api/admin/events/{id}/matches/generate` (admin role) that generates all round-robin matches for every group in the event. The system SHALL reject generation if the event is not `IN_PROGRESS` or has no groups, and SHALL clear any existing matches for the event first (idempotent re-generation while not locked / completed).
...
#### Scenario: Reject generation when event has no groups
- **WHEN** admin calls generate on an event with zero match_groups
- **THEN** the system returns 1006 INVALID_STATUS_TRANSITION (or PARAM_ERROR) and no matches are created

#### Scenario: Idempotent re-generation clears prior matches
- **WHEN** admin calls generate twice on the same event
- **THEN** the second call removes prior matches and re-creates from current group assignments (count matches the current assignment count, not the prior count)
...
The system SHALL validate every game in the submission against badminton rules:
...
#### Scenario: Reject invalid BO3 (e.g. 3-0 scoreboard but only 2 games submitted)
- **WHEN** games_won_a computed from submission != 2 AND != max(games)→ such that no one reached 2 wins
...
- **THEN** status returns to SCHEDULED and all score fields are null/zero

#### Scenario: Reset a not-yet-completed match
- **WHEN** admin resets a SCHEDULED match (no-op or allowed)
- **THEN** the system returns success (or a no-op acknowledgment)
...
- **WHEN** a user is in a team that plays 2 matches of event X (both as slot_a and slot_b)
- **THEN** the response includes both matches with opponent team info

### Requirement: Group standings (real-time)

The system SHALL provide `GET /api/app/events/{id}/standings` returning per-group standings ranked by: (1) wins desc, (2) games-won diff desc (sum of games_won_a - games_won_b across all matches), (3) head-to-head (the participant who won the match between the two ranks higher). Ties after all three criteria share the same rank. Standings are computed live from match data, not persisted.
...
#### Scenario: Standings tie broken by head-to-head
- **WHEN** two participants both have 1 win and 1 loss, and one beat the other directly
- **THEN** the direct winner ranks higher
