...
## Purpose
赛事完赛（COMPLETED）后，按赛事配置的名次加分表（rank→points）自动给每位参赛者写入 point_record（source=PLACEMENT）。双打时分数在队员间对半分。
...
- **THEN** two rows are written: userId=A points=50, userId=B points=50; if 100 is odd (e.g., 101), one gets 51 and the other 50
...
The issued rows SHALL carry `seasonCode = <code of the CURRENT season for the event.type>`. The system queries the `season` table for `status = 'CURRENT' AND type = <event.type>` and uses its `code`. If no current season exists for the type, `seasonCode` SHALL be `null` (rows still written — placement tolerates a missing season).
