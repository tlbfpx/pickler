...
- `error_msg` — VARCHAR(256) nullable; reused by R3 to store track event name (minimal schema principle)
- `created_at` — DATETIME(3) DEFAULT CURRENT_TIMESTAMP(3)
...
The WeChat mini-program client (`hey-pickler-wxapp`) MUST provide a `utils/tracker.js` module exposing `trackEvent(name, props)` and `trackError(msg, stack)`. The client SHOULD:

- Persist a stable `did` via `wx.getStorageSync('did')` on first launch (`did-<timestamp>-<random>`)
- Hook into `App.onLaunch` to emit `app_launch` (with `scene` from `getLaunchOptionsSync`)
- Hook into `App.onError` and `App.onUnhandledRejection` to emit `js_error` (msg ≤ 200, stack ≤ 500)
- Hook into `App.onHide` to emit `app_hide` (Phase 3 算前台时长)
- Hook into `utils/request.js` success and fail branches to emit `http_request` with `method/path/code/latencyMs`
- Apply `sanitizeProps()` to cap props at 50 keys and 512 chars per value before transmission

All client-to-server events MUST be best-effort: a failed `wx.request` SHALL be logged at WARN level and MUST NOT propagate to the calling code or interrupt the user-visible operation.
...
#### Scenario: Persistent device id across sessions
- **WHEN** a WeChat app launches for the second time
- **THEN** `wx.getStorageSync('did')` returns the same `did-<timestamp>-<random>` from the first launch (Phase 3 同期群分母需要稳定的 did 关联)

#### Scenario: Failed track request does not break user flow
- **WHEN** `trackEvent('event_view', ...)` is called and the underlying `wx.request` fails (network error)
- **THEN** `console.warn('[tracker] event failed: ...')` is emitted but the calling page's navigation / state update is unaffected